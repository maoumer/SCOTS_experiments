...
% initial state
x0=[0 10 0 0]; % must be within controller domain. See different ways of getting x0 below
...
while(1)
  ...
  v=[v; u(end,:)]; % u can use the first, last or any of the input options from line 55. The 3 images show the different results because of this
  [t x]=ode45(@unicycle_ode,[0 .3], y(end,:), [], u(end,:)); % same comment as line above.
...
% Targets
v=[8.6 0; 10.2 0; 8.6 1.1; 10.2 1.1];
patch('vertices',v,'faces',[1 2 4 3],'facec',colors(2,:),'edgec',colors(2,:));

% Obstacles
v=[1.6     0  ;4  0   ; 1.6     2    ; 4 2  ]; %{-1.8,4,-0, 2};
patch('vertices',v,'faces',[1 2 4 3],'facec',colors(1,:),'edgec',colors(1,:));
v=[1.6     4  ;4  4   ; 1.6     10    ; 4 10  ]; %{-1.8,4,-4,10};
patch('vertices',v,'faces',[1 2 4 3],'facec',colors(1,:),'edgec',colors(1,:));
v=[5.1   6.8  ;6.4  6.8   ; 5.1 7.2   ; 6.4 7.2  ]; %{-5.1,6,-6.8,7.2};
patch('vertices',v,'faces',[1 2 4 3],'facec',colors(1,:),'edgec',colors(1,:));
v=[6.4   0  ;8.4  0   ; 6.4 8    ; 8.4 8   ]; %{-6.4,8.2,-0,8};
patch('vertices',v,'faces',[1 2 4 3],'facec',colors(1,:),'edgec',colors(1,:));
