/*
 * diff_drive.cc - 1 target version, old obstacles (rungger version)
 *
 *  created on: 22.11.2023
 *      author: mohammed adib oumer (modified from rungger version for this specific dynamics)
 */
...
auto  diff_drive_post = [](state_type &x, input_type &u) -> void {
  ...
  /* the ode describing the diff_drive */
...
int main() {
  ...
  ts.writeToFile("diff_drive_target.bdd");
  ...
  controller.writeToFile("diff_drive_controller.bdd");
